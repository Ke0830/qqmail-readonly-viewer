---
name: 本地只读邮箱查看器
description: 面向快速扫读、账户切换与可靠定位的本地只读收件箱界面。
colors:
  primary: "#282828"
  dark-accent: "#d9d9d9"
  canvas: "#ffffff"
...
# Design System: 本地只读邮箱查看器

## Overview

**Creative North Star: "清晰的收件箱工作台"**

这是一个用于个人收件箱扫读的操作界面，而不是营销页面或拟物邮箱。页面优先让用户看清“正在看哪个账户、共有多少、自己在哪一页、下一步能做什么”，再以克制的层次和留白降低大量邮件带来的压力。

界面跟随系统浅色或深色模式：浅色采用纯白画布、浅灰控件与石墨黑主操作；深色采用石墨黑、深灰表面与灰白高光。两种模式都依靠中性明暗层级，而不是彩色强调，来区分当前筛选与主要操作。邮件行本身就是内容，不叠加无意义的卡片。缓存设置沿用同一套克制层级，只在需要明确清理数据时使用危险色边界。
...
- 仅通过状态、边线和有限阴影建立层级。
- 后台索引与账户级故障以不打断阅读的状态提示呈现，不自动刷新或移动当前页面。

## Colors

中性石墨层级服务长时间阅读，单一明暗强调服务于定位而非装饰。
...
- **每页数量：** 直接显示可点击的 30、50、100 标签，点击后立即应用并返回第一页；当前数量使用与筛选、账户一致的选中态。
- **Disabled:** 无可用目标时使用低对比但仍可辨认的静态控件，不伪装成可点击链接。

### 账户选择

- **单账户：** 账户地址和服务商显示在标题下，列表行不重复账户信息。
- **切换方式：** 两个及以上账户时直接显示可点击的账户标签，不使用需要二次确认的下拉表单；点击“全部账户”或账户名称立即切换，并保留当前邮件筛选与每页数量。
- **全部账户：** “全部账户”标签拥有明确选中态；每封邮件在发件人信息下标出账户名称和地址，避免不同邮箱 UID 或同名发件人混淆。
- **故障：** 汇总读取中某个账户失败时，在列表上方显示该账户级提示，不隐藏其他可读账户。

### 缓存设置

- **入口：** 与“刷新列表”并列，但保持次级按钮层级。
- **结构：** 设置、缓存清理和本机位置分成少量完整区块；不使用装饰性仪表盘或虚构统计。
- **降级确认：** 从正文缓存切换到更低模式时，必须在独立确认状态中明确选择“保留旧缓存”或“清除不再允许的数据”。
- **危险操作：** “重建全部缓存”使用危险色文字和边界；“清除正文缓存”保持普通次级操作。
- **窄屏：** 标签和输入改为单列，主次操作自然换行，所有字段保持可见焦点。
