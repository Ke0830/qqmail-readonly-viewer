...
**让 Agent 以只读方式筛选和汇总邮件，并简洁地向你汇报，避免重要信息淹没在广告与垃圾邮件中。**
...
- 邮件中的远程图片和 CID 图片默认不加载，只显示受限占位；网页链接必须确认完整地址后才会打开。
- 邮件列表优先读取本机 SQLite 缓存；默认只把用户实际打开过的纯文本和清洗后 HTML 加密缓存到本机，不会预取全部正文。
...
- `body`：默认模式；持久化元数据，并使用 AES-GCM 加密缓存用户实际打开过的纯文本与清洗后安全 HTML。随机加密密钥只保存在 macOS 钥匙串或 Windows 凭据管理器；原始 HTML 不会写入缓存。
...
- 详情页先读取邮件头和 `BODYSTRUCTURE`，再按 MIME 树选择一个明确的非附件正文 section。网页优先读取 HTML 正文，必要时降级为纯文本；CLI 和现有 JSON 接口始终输出纯文本。任何路径都不会请求图片、附件 payload 或完整邮件。
- 原始 HTML 只在内存中短暂存在，不会直接渲染或写入缓存。网页仅显示经过严格清洗的安全 HTML：脚本、表单、事件属性、自动跳转、危险链接以及可发起网络请求的样式都会被移除，并在受限 iframe 中通过独立 CSP 隔离。
- 远程图片和 CID 图片均不会加载。普通图片显示文字占位，无意义的跟踪图直接移除；页面会提示本邮件隐藏的图片数量。邮件中的 `http` / `https` 链接必须先在应用内确认完整地址，才会以新标签页打开。
- 附件只从 MIME 结构读取名称，不会下载、打开或保存附件内容；无法安全识别正文结构时，查看器会拒绝读取正文，而不会回退下载整封原始邮件。
- SQLite 中的邮件元数据（包括主题、发件人和附件名称）不是正文加密的一部分；默认 `body` 模式下，只有按需缓存的纯文本和清洗后安全 HTML 使用 AES-GCM 加密。缓存密钥与邮箱凭据分开保存在系统凭据库。
- 在不加载邮件图片的前提下，网页无法与原邮箱做到像素级一致；表格、颜色、按钮和文字层级通常可以大部分恢复。
- 不读取垃圾箱、自定义文件夹或已发送邮件。
