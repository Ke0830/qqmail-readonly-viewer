...
每个账户的凭据、可选的翻译 API Key 和正文缓存密钥保存在本机系统凭据存储中；查看器通过加密 IMAPS 增量同步元数据，从本机 SQLite 提供单账户或汇总列表，并在网页进入后按日期倒序后台预取安全正文与正文图片。用户可按需把当前邮件翻译为简体中文；产品不提供删除、发送或修改邮件状态的能力。
...
- 支持仅内存、持久化元数据、加密正文与图片三种缓存模式，以及 0–1440 分钟同步间隔。
- 默认正文缓存使用 AES-GCM；进入网页后按日期从新到旧、账户间并行预取全部已读与未读正文及通过安全校验的正文图片。
- 详情页支持用户主动调用 DeepL 或 OpenAI 兼容 API 翻译主题与可见正文；译文保留安全 HTML 结构并按缓存模式加密保存，不在后台自动翻译。
