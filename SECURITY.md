...
In the default `body` mode, entering the web view starts a newest-first background prefetch of every indexed message body and its approved body images, regardless of read state. The versioned encrypted record can contain the plain-text representation and strictly sanitized HTML, but never the original HTML. Bodies and approved images are encrypted with AES-GCM and authenticated with the account name and UID. `metadata` mode never persists or prefetches bodies. `memory` mode performs the same prefetch into an in-process SQLite database and creates no cache file. The persistent database uses WAL mode and is stored under the current user's local cache directory.
